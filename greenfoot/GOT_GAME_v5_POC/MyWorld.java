...
public class MyWorld extends World
{
    ...
    private static double scrollSpeed = 2;
    ...
    {   
        ...
        List<RivalX> rivals = getObjects(RivalX.class);
        for (RivalX r : rivals) {
            r.setLocation(r.getX()-(int)this.scrollSpeed/2, r.getY());
        }
        bg.drawImage(bgBase, position, 0);
        bg.drawImage(bgImage, position + picWidth, 0);
    } 
    ...
    {   
        ...
        player = new Player();
        addObject(player,90,432);
        player.setLocation(290,432);
        createRivals(790);
        mediator = new PlayerMediator();
        addObject(mediator,0,0);
        mediator.setState(mediator.getSuperState());
        healthBar = new HealthBar();
        addObject(healthBar,500,56);
        
        gameProgress = new GameProgress();
        addObject(gameProgress,320,20);
        Castle castle = new Castle();
        addObject(castle,280,20);
        
        HealthIcon healthIcon = new HealthIcon();
        addObject(healthIcon,280,76);
